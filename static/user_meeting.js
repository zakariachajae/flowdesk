document.addEventListener("DOMContentLoaded", async function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('user_dashoard').addEventListener('click', async function (event) {
        ...
        document.getElementById('user_meeting').addEventListener('click', async function (event) {
            event.preventDefault(); // Prevent default link behavior
            const response = await fetch('/user_meeting');
            if (response.ok) {
                const html = await response.text();
                document.getElementById('content').innerHTML = html; // Update the content area
            } else {
...
async function loadmeetings() {
    try {
        const response = await fetch("/user_meetings?user_id=18"); // Fetch meetings data
        const meetings = await response.json(); // Parse JSON response

        const meeting_cards = document.getElementById("user_meeting_cards");
